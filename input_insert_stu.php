...
<html>
    ...
    <script>
        function newFunc(){
            var newVar = document.getElementById('school').value;
            window.location.href = "?school="+newVar;
            //window.location.href = "./input_insert_stu.php?school="+newVar;
        }
    </script>
    <?php
        ...
        /*function programList(){
            ...
            global $conn;
            $sql="SELECT ProgramID, ProgramName FROM Program WHERE SchoolID = $school_id";
            $result = mysqli_query($conn,$sql);
            while($row = mysqli_fetch_assoc($result)){
                echo "<option value=\"".$row['ProgramID']."\">".$row['ProgramName']."</option><br>";
            }
        }*/
        function programList(){
            global $conn;
            $sql="SELECT ProgramID, ProgramName FROM Program";
            $result = mysqli_query($conn,$sql);
            while($row = mysqli_fetch_assoc($result)){
                echo "<option value=\"".$row['ProgramID']."\">".$row['ProgramName']."</option><br>";
            }
    ...
    ?>
        ...
        <form action="insert_stu.php" method="post">
            <div>
                <label for="sid">Student ID:</label> <input type="text" name="eid" id="eid"> <br>
                <label for="prefix">Prefix:</label> <input type="text" name="prefix" id="prefix"> <br>
                <label for="fname">First Name:</label> <input type="text" name="fname" id="fname"> <br>
                <label for="lname">Last Name:</label> <input type="text" name="lname" id="lname"> <br>
                <label for="school">School:</label> <select name="school" id="school" style="text-align: center;" onchange="newFunc()">
                <?php printScool() ?>
                </select><br>
                <label for="program">Program:</label> <select name="program" id="program">
                </select><br>
                <label for="dean">Dean:</label> <input type="text" name="dean" id="dean"> <br>
                <label for="advisor">Advisor:</label> <input type="text" name="advisor" id="advisor"> <br>
                <label for="gpax">GPAX:</label> <input type="number" min="0" max="4" step="0.1" name="gpax" id="gpax"> <br>
